...
public sealed class SqliteMigrationTests
{
    ...
    public async Task EmptyDatabasesCreateIndependentLedgersAndAreIdempotent()
    {
        ...
        foreach (var database in Enum.GetValues<DatabaseKind>())
        {
            ...
            Assert.Equal(database == DatabaseKind.App ? 2L : 3L, await Scalar(connection, "SELECT COUNT(*) FROM SchemaMigrations;"));
            Assert.Equal(database == DatabaseKind.App ? 7L : 9L, await Scalar(connection, "SELECT COUNT(*) FROM sqlite_master WHERE type='table';"));
        }
