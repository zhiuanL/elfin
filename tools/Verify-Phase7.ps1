...
$artifactRoot = Join-Path $repoRoot "artifacts/Phase7/$Configuration"
...
try {
    ...
    & $dotnetExe test DesktopPet.sln -c $Configuration --no-build --no-restore -p:ArtifactsPath=$artifactRoot `
        --logger 'trx;LogFilePrefix=phase-7' --results-directory "artifacts/TestResults/Phase7/$Configuration"
