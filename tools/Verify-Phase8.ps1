...
$artifactRoot = Join-Path $repoRoot "artifacts/Phase8/$Configuration"
...
try {
    ...
    & $dotnetExe test DesktopPet.sln -c $Configuration --no-build --no-restore -p:ArtifactsPath=$artifactRoot --logger 'trx;LogFilePrefix=phase-8' --results-directory "artifacts/TestResults/Phase8/$Configuration"
